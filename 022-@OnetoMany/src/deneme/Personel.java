...
public class Personel implements Serializable {
	...
	@OneToMany // 1-M BAÐLANTI @JoinColumn(name="PERSONEL_ID")
	@JoinTable(name="PERSONEL_MESLEK",    // ÝKÝ ENTITY ARASINDAKÝ ÝLÝÞKÝ
				joinColumns= @JoinColumn(name="PERSONEL_ID"), // ANA TABLODAKI ID 
				inverseJoinColumns = @JoinColumn(name="MESLEK_ID") // CHILD TABLODAKI ID
	)
	private Collection<Meslek> meslek = new ArrayList<Meslek>();
	// @JoinColumn(name="PERSONEL_ID") -> TEK BÝR KOLON OLUÞUR.
	// @JoinTable(name="PERSONEL_MESLEK" -> TEK BR TABLO OLUÞUR.
